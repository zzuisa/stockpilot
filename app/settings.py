...
def _futu_port() -> int:
    # 注意：同名 k8s Service(futu-opend) 会注入 FUTU_OPEND_PORT=tcp://ip:11111，
    # 取末尾端口号，避免 int() 崩溃(容器显式 env 未覆盖时的兜底)。
    v = _get("FUTU_OPEND_PORT", "11111") or "11111"
    if ":" in v:
...
_futu_host = _get("FUTU_OPEND_HOST", "127.0.0.1")
# 同理防御 FUTU_OPEND_HOST 被注入成 tcp://ip:port 形式
if "://" in _futu_host:
    _futu_host = _futu_host.split("://", 1)[-1].rsplit(":", 1)[0]
FUTU_OPEND_HOST = _futu_host or "127.0.0.1"
FUTU_OPEND_PORT = _futu_port()
